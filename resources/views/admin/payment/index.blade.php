                ...
                <div id="{{$payment->id}}type_edit_Modal" class="modal fade" role="dialog">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      {{ html()->modelForm($payment, 'PATCH', action('AdminPaymentController@update', [$payment->id]))->open() }}
                        <div class="modal-body">
                          <div class="row">
                            <div class="col-md-6">
                              <div class="form-group{{ $errors->has('discount') ? ' has-error' : '' }}">
                                {{ html()->label('Discount', 'discount') }}
                                {{ html()->text('discount')->class('form-control')->placeholder('Enter Discount') }}
                                <small class="text-danger">{{ $errors->first('discount') }}</small>
                              </div>
                              <div class="form-group{{ $errors->has('payment_mode_id') ? ' has-error' : '' }}">
                                {{ html()->label('Payment Mode', 'payment_mode_id') }}
                                {{ html()->select('payment_mode_id', array('' => 'Chooes Payment Mode') + $payment_mode_lists)->class('form-control select2') }}
                                <small class="text-danger">{{ $errors->first('payment_mode_id') }}</small>
                              </div>
                              <div class="form-group{{ $errors->has('advance') ? ' has-error' : '' }}">
                                {{ html()->label('Advance Payment', 'advance') }}
                                {{ html()->text('advance')->class('form-control')->placeholder('Enter Payment In Advance') }}
                            ...
                            <div class="col-md-6">
                              <div class="form-group{{ $errors->has('remark') ? ' has-error' : '' }}">
                                ...
                                {{ html()->textarea('remark')->class('form-control')->rows('8')->placeholder('Enter Remark') }}
                ...
                <div id="{{$payment->id}}type_del_Modal" class="delete-modal modal fade" role="dialog">
                  <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        {{ html()->form('DELETE', action('AdminPaymentController@destroy', [$payment->id]))->open() }}
